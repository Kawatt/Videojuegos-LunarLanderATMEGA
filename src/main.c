...
int main() {
    ...
    struct Dibujable* terreno = crear_dibujable(&Terreno);
    for(;;) {
        //dibujar_dibujable(terreno);
        //dibujar_dibujable(nave);
...
void repintar_ventana() {
    //InvalidateRect(NULL, TRUE); // Enviar mensaje WM_PAINT para repintar
}
